...
{
    int total_bits = L2_SIZE - 4 * L2_FP - 4 - 2;
    ...
    case 2:
        ...
        l_size += (t_size + L2_FP + 1) - (t_size + L2_FP) / 3 * 2;
        return new int[3]{l_size, m_size, s_size};
    case 1:
        m_size += (t_size + s_size + 2 * L2_FP + 2) / 2;
        l_size += (t_size + s_size + 2 * L2_FP + 2) - (t_size + s_size + 2 * L2_FP + 2) / 2;
        return new int[2]{l_size, m_size};
    case 0:
        l_size += t_size + s_size + m_size + 3 * L2_FP + 3;
